...
async def work(session: ClientSession):
    while True:
        ...
        async with session.get('https://api.spotify.com/v1/me/player/currently-playing', headers=oauth) as response:
            ...
            # 401 means our access token is expired, so we need to refresh it
                ...
                async with session.post("https://accounts.spotify.com/api/token", data=data) as post_response:
            ...
            # catch anything else
                ...
                logger.error(f"Spotify, error {str(status_code)}, text: {await response.text()}")
